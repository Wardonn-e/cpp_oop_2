...
ostream& workers::operator<< (std::ostream& out, const EmployeePtr& other) {
	...
	std::string type[] = { "Ðàñ÷åòíûé", "Âêëàäîâûé", "Êðåäèòíûé"};
	out << type[static_cast<int>(other->get_type())] << " ñ÷åò " << other->get_name() << "\n" << "Áàëàíñ: " << other->get_balance() << " | Ïðîöåíòíàÿ ñòàâêà - " << other->get_percent() << "\n" << "Áàëàíñ â ñëåäóþùåì ìåñÿöå: " << other->calcul() << endl << endl;
	return out;
}